...
if [[ -z "${JAVA_HOME:-}" || ! -d "${JAVA_HOME}/include" || ! -d "${JAVA_HOME}/include/darwin" ]]; then
  echo "::error file=${SCRIPT_PATH}/compile_macosx_x86_64.sh,line=15::Could not find JNI headers. Set JAVA_HOME to a full macOS JDK." >&2
  exit 1
fi
...
if [[ -z "${HDF5_PACKAGE_PRESET}" ]]; then
  echo "::error file=${SCRIPT_PATH}/compile_macosx_x86_64.sh,line=38::No matching HDF5 build preset directory found for ${HDF5_CMAKE_PRESET} under ${BUILDDIR}/hdf5-${VERSION}/build110" >&2
  exit 1
fi
...
if [[ -z "${HDF5_LINK_LIB}" ]]; then
  echo "::error file=${SCRIPT_PATH}/compile_macosx_x86_64.sh,line=46::Could not find a CMake-built libhdf5 dylib under ${BDIR}/bin" >&2
  exit 1
fi
...
DEPLOY_DIR="../../../libs/native/jhdf5/x86_64-Mac OS X"
mkdir -p "$DEPLOY_DIR"
