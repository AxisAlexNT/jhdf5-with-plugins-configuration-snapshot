...
if [[ ! -z $CMAKE_HDF5 ]]; then
	...
	if [[ -f "$PLUGIN_TAR" ]]; then
		CMAKE_EXTRA_ARGS+=(
			...
			"-DENABLE_LZF:BOOL=ON"
			"-DENABLE_BSHUF:BOOL=ON"
			"-DENABLE_LZ4:BOOL=ON"
			"-DENABLE_ZSTD:BOOL=ON"
			"-DHDF5_ENABLE_LZF_SUPPORT:BOOL=ON"
			"-DHDF5_ENABLE_BSHUF_SUPPORT:BOOL=ON"
			"-DHDF5_ENABLE_LZ4_SUPPORT:BOOL=ON"
			"-DHDF5_ENABLE_ZSTD_SUPPORT:BOOL=ON"
		)
	fi
